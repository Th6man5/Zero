...
var culori = (() => {
  ...
  var useMode = (definition28) => {
    ...
    (definition28.parse || []).forEach((parser) => {
      if (typeof parser === "function") {
        ...
        colorProfiles[parser] = definition28.mode;
      }
    });
  ...
  var convertXybToRgb = ({ x, y, b, alpha }) => {
    ...
    const res = convertLrgbToRgb_default({
      r: 11.031566904639861 * l - 9.866943908131562 * m - 0.16462299650829934 * s2,
      g: -3.2541473810744237 * l + 4.418770377582723 * m - 0.16462299650829934 * s2,
      b: -3.6588512867136815 * l + 2.7129230459360922 * m + 1.9459282407775895 * s2
    });
    if (alpha !== void 0)
      res.alpha = alpha;
    return res;
  ...
  var mono = (arr) => {
    ...
    for (let i = 0; i < n3; i++) {
      ...
      yp.push(
        i > 0 ? (sgn(s2[i - 1]) + sgn(s2[i])) * min(abs2(s2[i - 1]), abs2(s2[i]), 0.5 * abs2(p4[i])) : void 0
      );
    }
    return [s2, p4, yp];
  };
